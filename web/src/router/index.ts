...
router.beforeEach(async (to, from, next) => {
	...
	if (to.path === '/login') {
		...
		if ((useUserInfo(pinia).userInfos.pwd_change_count ?? 0) > 0) {
			next('/home');
		} else {
			next();
		}
